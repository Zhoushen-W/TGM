...
def compute_HM(img, prob=None, patch_size=None, num_bins=16, eps=1e-9, norm_mode=None, a=10, b=1):
    ...
    #     bins = torch.clamp((p_norm * (num_bins - 1)).long(), 0, num_bins - 1)
    #     hist = torch.zeros(b, num_bins, device=device)
    #     for k in range(num_bins):
    #         hist[:, k] = (bins == k).float().sum(dim=1)
    #     prob_hist = hist / (hist.sum(dim=1, keepdim=True) + eps)
    #     entropy[:, i] = -torch.sum(prob_hist * torch.log(prob_hist + eps), dim=1)
    ...
    if norm_mode is None:
        ...
        ValueError("norm_mode except sigmoid is not support currently")
